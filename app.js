...
const STORE='b2-native-cards-extra-v12';
...
const els={
  ...
  verbBox:$('verbBox'),inf:$('inf'),pres3:$('pres3'),past:$('past'),perf:$('perf'),plus:$('plus'),syn:$('syn'),ex:$('ex'),
...
function chipsFor(c,lang){
  if(lang==='de') return (c.synonyms_de||[]).slice(0,3);
  if(lang==='fa') return (c.synonyms_fa||[]).slice(0,3);
  return (c.synonyms_en||c.synonyms||[]).slice(0,3).map(cleanEnglish);
}
function chipLabel(lang){
  if(lang==='de')return 'German related meanings';
  if(lang==='fa')return 'Dari related meanings';
  return 'English related meanings';
}
function renderChips(c,lang='en'){
  const chips=chipsFor(c,lang);
  const html=(chips.length?chips:['—']).map(s=>`<span>${esc(s)}</span>`).join('');
...
function cleanSpeechText(text, lang){
  ...
  if(lang==='en')t=cleanEnglish(t); if(lang==='de')t=t.replace(/\s*\/\s*/g,' oder '); if(lang==='fa')t=t.replace(/[؛,،]/g,'، ');
  return t;
}
function formStep(c){
  const f=c.forms||{};
  ...
  if(c.category==='verb'){
    const visible=[],spoken=[]; for(const k of ['infinitive','past','perfect','plusquamperfekt'])if(f[k]){visible.push(f[k]);spoken.push(f[k]);}
...
function render(){const c=filtered[idx];if(!c){els.frontText.textContent='No cards';els.frontHint.textContent='Change filters.';els.frontSub.textContent='';renderProgress();return}const f=getManualFront(c);lastFront=f;renderDetails(c,f.lang);setDirForLang(f.lang);els.frontText.textContent=f.display||'—';els.frontHint.textContent=f.label;els.frontSub.textContent=f.sub||'';els.answer.classList.toggle('hidden',!flipped);els.card.classList.toggle('playing',playing);renderProgress();}
...
function voices(){return ('speechSynthesis'in window)?(speechSynthesis.getVoices()||[]):[]}function isMobile(){return /iPhone|iPad|iPod|Android/i.test(navigator.userAgent||'')}function voiceLabel(v){return v?`${v.name} (${v.lang})`:'No matching voice found'}
function bestVoice(lang){const vs=voices(),premium=/premium|enhanced|natural|siri|google|microsoft|neural|online/i;if(lang==='de')return vs.find(v=>v.lang==='de-DE'&&premium.test(v.name))||vs.find(v=>v.lang==='de-DE')||vs.find(v=>v.lang&&v.lang.startsWith('de'));if(lang==='en')return vs.find(v=>v.lang==='en-US'&&premium.test(v.name))||vs.find(v=>v.lang==='en-US')||vs.find(v=>v.lang&&v.lang.startsWith('en'));const faName=/dari|persian|farsi|فارسی|دری/i;return vs.find(v=>v.lang==='fa-AF')||vs.find(v=>v.lang==='fa-IR')||vs.find(v=>v.lang&&v.lang.toLowerCase()==='fa')||vs.find(v=>faName.test(v.name||''))||vs.find(v=>v.lang&&v.lang.startsWith('fa'))||vs.find(v=>v.lang&&v.lang.startsWith('ar'));}
function candidateLangs(lang){if(lang==='de')return ['de-DE','de'];if(lang==='en')return ['en-US','en-GB','en'];return ['fa-AF','fa-IR','fa','ar-SA','ar'];}
function speakWithFallback(text,lang='de',done=()=>{},cancelFirst=true){if(!text){done();return}if(!('speechSynthesis'in window)){showWarning('Speech is not supported in this browser.');done();return}clearWarning();if(cancelFirst)speechSynthesis.cancel();const cleaned=cleanSpeechText(text,lang),langs=candidateLangs(lang),picked=bestVoice(lang);let attempt=0,completed=false;function attemptSpeak(){const u=new SpeechSynthesisUtterance(cleaned),chosen=attempt===0?picked:null;u.lang=chosen?.lang||langs[Math.min(attempt,langs.length-1)];u.rate=lang==='de'?0.64:0.92;u.pitch=1;if(chosen)u.voice=chosen;u.onend=()=>{completed=true;done()};u.onerror=()=>{if(lang==='fa'&&attempt<langs.length-1){attempt++;setTimeout(attemptSpeak,100)}else{completed=true;if(lang==='fa')showWarning('Dari audio is not available from this phone/browser. Text works. Guaranteed mobile Dari audio needs bundled audio files or cloud TTS.');done();}};speechSynthesis.speak(u);if(lang==='fa'&&isMobile())setTimeout(()=>{if(!completed&&!speechSynthesis.speaking&&!speechSynthesis.pending)showWarning('This phone did not start a Dari/Persian voice. It likely has no exposed Dari/Persian browser voice.');},1400)}attemptSpeak();}
function say(t,l='de',d=()=>{}){speakWithFallback(t,l,d,true)}function queueSay(t,l='de',d=()=>{}){speakWithFallback(t,l,d,false)}function speakFront(){if(lastFront)say(lastFront.speech||lastFront.display,lastFront.lang)}
function unlockSpeech(){if(!('speechSynthesis'in window)){showWarning('Speech is not supported.');return}speechSynthesis.cancel();const u=new SpeechSynthesisUtterance('ok');u.volume=.01;u.lang='en-US';u.onend=()=>{clearWarning();updateVoiceStatus();els.now.textContent='Mobile speech enabled.'};u.onerror=()=>showWarning('Speech unlock failed.');speechSynthesis.speak(u)}
...
function playSelected(){if(!filtered.length)return;unlockSpeech();stop(false);playing=true;paused=false;playQueue=[];const source=[...filtered],rep=Number(els.repeat.value)||1;for(let r=0;r<rep;r++)source.forEach((c,i)=>{const parts=cardScript(c);if(parts.length)playQueue.push({card:c,parts,cardNo:i+1,totalCards:source.length})});playIndex=0;setTimeout(()=>{if('speechSynthesis'in window)speechSynthesis.cancel();playNextPart(0)},230)}
function playNextPart(partIdx){if(!playing||playIndex>=playQueue.length){stop();return}if(paused)return;const item=playQueue[playIndex];if(partIdx>=item.parts.length){playIndex++;playNextPart(0);return}const part=item.parts[partIdx];renderPlaybackStep(item,partIdx);setTimeout(()=>queueSay(part.speech||part.display,part.lang,()=>setTimeout(()=>playNextPart(partIdx+1),520)),170)}
function pauseResume(){if(!playing||!('speechSynthesis'in window))return;if(paused){paused=false;els.pauseList.textContent='Pause';speechSynthesis.resume()}else{paused=true;els.pauseList.textContent='Resume';speechSynthesis.pause();els.now.textContent='Paused.'}}function stop(doRender=true){playing=false;paused=false;playQueue=[];playIndex=0;if('speechSynthesis'in window)speechSynthesis.cancel();els.pauseList.textContent='Pause';els.now.textContent='Ready.';els.card.classList.remove('playing');els.frontText.removeAttribute('dir');if(doRender)render();}
function detectDevice(){let d='Desktop/laptop';const ua=navigator.userAgent||'';if(/iPhone|iPad|iPod/i.test(ua))d='iPhone/iPad';else if(/Android/i.test(ua))d='Android';else if(/Windows/i.test(ua))d='Windows PC';else if(/Macintosh/i.test(ua))d='Mac';els.deviceInfo.textContent=`Detected: ${d}. The app uses the best available browser/device voice.`}
function updateVoiceStatus(){if(!('speechSynthesis'in window)){els.voiceDe.textContent=els.voiceEn.textContent=els.voiceFa.textContent='Speech not supported';return}const vs=voices();els.voiceDe.textContent=voiceLabel(bestVoice('de'));els.voiceEn.textContent=voiceLabel(bestVoice('en'));const fa=bestVoice('fa');els.voiceFa.textContent=fa?voiceLabel(fa):`No Dari/Persian/Farsi voice found among ${vs.length} voices`;}
function addCard(e){e.preventDefault();const d=Object.fromEntries(new FormData(els.addForm).entries());const syns=String(d.synonyms||'').split(',').map(s=>s.trim()).filter(Boolean).slice(0,3);while(syns.length<3)syns.push(syns.length?'related meaning':'key term');const c={id:'custom-'+Date.now(),source:'user',list:'My cards',unit:d.unit||'My list',part:'',category:d.category||'other',german:d.german,english:d.english,dari:d.dari,article:d.article||'',singular:d.german,plural:d.plural||'',forms:{infinitive:d.infinitive||'',present3:'',past:d.past||'',perfect:d.perfect||'',plusquamperfekt:d.plusquamperfekt||''},synonyms:syns,synonyms_en:syns,synonyms_de:syns,synonyms_fa:syns,example:''};extra.push(c);localStorage.setItem(STORE,JSON.stringify(extra));cards=[...initialData,...extra];setup();apply();els.addForm.reset()}
