// v37: remove old service workers/caches once so old broken versions cannot control audio.
(function(){try{const key='v37AudioResetDone';if(!sessionStorage.getItem(key)){sessionStorage.setItem(key,'1');Promise.all([('serviceWorker'in navigator)?navigator.serviceWorker.getRegistrations().then(rs=>Promise.all(rs.map(r=>r.unregister()))):Promise.resolve(),('caches'in window)?caches.keys().then(ks=>Promise.all(ks.map(k=>caches.delete(k)))):Promise.resolve()]).then(()=>{if(!location.search.includes('fresh37=')){const sep=location.search?'&':'?';location.replace(location.pathname+location.search+sep+'fresh37='+Date.now())}}).catch(()=>{});}}catch(e){}})();
const initialData = window.FLASHCARD_DATA.cards;
const STORE='b2-native-cards-extra-v37';
...
let packAudio=null; let packStopTimer=null; let currentPackId='';
...
const els={
  ...
  testDe:$('testDe'),testEn:$('testEn'),testFa:$('testFa'),unlockSpeech:$('unlockSpeech'),packStatus:$('packStatus'),audioMode:$('audioMode')
};
...
function getManualFront(c){
  ...
  if(m==='random')m=['german','english','dari','plural'][Math.floor(Math.random()*4)];
  if(m==='english')return{display:displayEnglish(c),speech:cleanSpeechText(c.english,'en'),label:'English',sub:'Medium',lang:'en',key:`${c.id}__en`};
  if(m==='dari')return{display:displayDari(c),speech:displayDari(c),label:'Dari',sub:'Medium',lang:'fa',key:`${c.id}__fa`};
  if(m==='plural'){const fs=formStep(c);return fs?{...fs,key:`${c.id}__forms`}:{display:'No plural/forms',speech:'',label:'Plural / forms',sub:'',lang:'de',key:''}}
  return{display:displayGerman(c),speech:displayGerman(c),label:'German',sub:'Slow',lang:'de',key:`${c.id}__de`}
}
...
// v37 classic v3-v6 voice engine for all languages.
// No WebAudio, no local sprite, no remote TTS, no automatic fallback switching.
// v37 audio engine:
// 1) high-quality generated local audio packs when available,
// 2) browser SpeechSynthesis fallback only when a pack is missing or user selects Browser only.
...
function pickVoice(lang){
  ...
  return vs.find(v=>v.lang===exact&&/premium|enhanced|natural|neural|siri|google|microsoft/i.test(v.name))||
...
function audioMode(){return els.audioMode?els.audioMode.value:'packsFirst'}
function saveAudioMode(){try{localStorage.setItem('audio-mode-v37',audioMode())}catch(e){}}
function loadAudioMode(){try{const m=localStorage.getItem('audio-mode-v37');if(m&&els.audioMode)els.audioMode.value=m}catch(e){}}
function packData(){return window.AUDIO_PACKS||{packs:{},keys:{}}}
function findAudioSegment(key){
  if(!key)return null;
  const a=packData();
  const packId=(a.keys||{})[key];
  const pack=packId&&(a.packs||{})[packId];
  const seg=pack&&(pack.segments||{})[key];
  return pack&&seg?{packId,pack,seg}:null;
}
function stopPackAudio(){
  clearTimeout(packStopTimer);
  if(packAudio){try{packAudio.pause()}catch(e){}}
}
function playPackSegment(key,done=()=>{}){
  const item=findAudioSegment(key);
  if(!item)return false;
  stopPackAudio();
  if(!packAudio){
    packAudio=new Audio();
    packAudio.preload='auto';
    packAudio.playsInline=true;
  }
  const {packId,pack,seg}=item;
  if(currentPackId!==packId || !packAudio.src.includes(pack.url.split('?')[0])){
    packAudio.src=pack.url;
  ...
  function finish(){
    ...
    finished=true;
    clearTimeout(packStopTimer);
    try{packAudio.pause()}catch(e){}
    done();
  }
  try{
    packAudio.currentTime=Math.max(0,Number(seg.s)||0);
    const duration=Math.max(.45,Number(seg.d)||2.0);
    packAudio.ontimeupdate=()=>{if(packAudio.currentTime>=Number(seg.s)+duration)finish()};
    packAudio.onended=finish;
    packAudio.onerror=()=>{showWarning('Audio pack failed to play. Falling back to browser voice.');finish()};
    const p=packAudio.play();
    if(p&&p.catch)p.catch(()=>{showWarning('Phone blocked audio. Tap Play/Test again.');finish()});
    packStopTimer=setTimeout(finish,Math.ceil((duration+.25)*1000));
    return true;
  }catch(e){
    showWarning('Audio pack failed: '+(e&&e.message?e.message:'unknown error'));
    return false;
  }
}
function sayBrowser(text, lang='de', done=()=>{}){
  if(!text){done();return}
  if(!('speechSynthesis'in window)){showWarning('Browser speech is not supported here.');done();return}
  const u=new SpeechSynthesisUtterance(text);
  u.lang=lang==='de'?'de-DE':lang==='en'?'en-US':'fa-AF';
  u.rate=lang==='de'?0.78:lang==='en'?0.86:0.90;
  u.pitch=1;
...
function say(text, lang='de', done=()=>{}, key=''){
  clearWarning();
  if(!text){done();return}
  const mode=audioMode();
  if(mode!=='browserOnly' && key && findAudioSegment(key)){
    if(playPackSegment(key,done))return;
  }
  if(mode==='packsOnly'){
    showWarning('No generated local audio pack for this item yet. Generate/upload audio packs first.');
    setTimeout(done,900);
    return;
  }
  sayBrowser(text,lang,done);
}
function queueSay(t,l='de',d=()=>{},key=''){say(t,l,d,key)}
function speakFront(){if(lastFront)say(lastFront.speech||lastFront.display||lastFront.txt,lastFront.lang,()=>{},lastFront.key||'')}
function unlockSpeech(){updateVoiceStatus();els.now.textContent='Audio engine refreshed.'}
function detectDevice(){
  let d='Desktop/laptop';const ua=navigator.userAgent||'';
  if(/iPhone|iPad|iPod/i.test(ua))d='iPhone/iPad';else if(/Android/i.test(ua))d='Android';else if(/Windows/i.test(ua))d='Windows PC';else if(/Macintosh/i.test(ua))d='Mac';
  els.deviceInfo.textContent=`Detected: ${d}. v37 uses generated local audio packs first; browser speech is only fallback.`;
}
function updatePackStatus(){
  const a=packData();
  const packCount=Object.keys(a.packs||{}).length;
  const segCount=Object.keys(a.keys||{}).length;
  if(els.packStatus)els.packStatus.textContent=packCount?`${packCount} packs · ${segCount} audio segments loaded`:'No generated packs loaded yet — browser fallback active';
}
function updateVoiceStatus(){
  if(!('speechSynthesis'in window)){els.voiceDe.textContent=els.voiceEn.textContent=els.voiceFa.textContent='Browser fallback not supported'}
  else{els.voiceDe.textContent=voiceLabel(pickVoice('de'));els.voiceEn.textContent=voiceLabel(pickVoice('en'));els.voiceFa.textContent=voiceLabel(pickVoice('fa'))}
  updatePackStatus();
}
function cardScript(c){
  ...
  if(els.playDe.checked){
    ...
    steps.push({t:g,l:'de',label:'German',sub:'Slow',key:`${c.id}__de`});
  }
  if(els.playForms.checked){
    const fs=formStep(c);
    if(fs)steps.push({t:fs.display||fs.speech,l:'de',label:fs.label||'German forms',sub:fs.sub||'Slow',key:`${c.id}__forms`});
  }
  if(els.playEn.checked&&c.english){
    steps.push({t:displayEnglish(c),l:'en',label:'English',sub:'Medium',key:`${c.id}__en`});
  }
  if(els.playFa.checked&&c.dari){
    const d=displayDari(c);
    steps.push({t:d,l:'fa',label:'Dari',sub:'Medium',key:`${c.id}__fa`});
  }
...
function renderPlaybackCard(item,partIdx){
  ...
  lastFront={display:p.t,speech:p.t,lang:p.l,label:p.label,sub:p.sub||'',key:p.key||''};
...
function playNextPart(partIdx){
  ...
  setTimeout(()=>say(txt,lang,()=>setTimeout(()=>playNextPart(partIdx+1),450),p.key||''),120);
}
