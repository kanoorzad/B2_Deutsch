...
const STORE='b2-native-cards-extra-v22';
...
function twoSynonyms(c,lang='en'){
  if(c.category!=='verb')return [];
  if(lang==='de')return (c.synonyms_de||[]).filter(Boolean).slice(0,3);
  if(lang==='fa')return (c.synonyms_fa||[]).filter(Boolean).slice(0,3);
  return ((c.synonyms_en&&c.synonyms_en.length?c.synonyms_en:c.synonyms)||[]).filter(Boolean).slice(0,3);
}
...
function renderSynonyms(c){
  const syns=twoSynonyms(c,lastFront?.lang||'en');
...
function renderPlaybackCard(item, partIdx){
  ...
  const syns=twoSynonyms(c,lastFront?.lang||'en');
  const synLine=syns.length?` · ${syns.join(' / ')}`:'';
