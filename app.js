...
const STORE='b2-native-cards-extra-v23';
...
// v23 Dari voice fix on v20 base:
// German/English stay on stable v6-style Web Speech.
// Dari now uses mobile-aware browser voice selection only; the v20 online-audio experiment is removed.
function voices(){return ('speechSynthesis'in window)?(speechSynthesis.getVoices()||[]):[]}
function isMobile(){return /iPhone|iPad|iPod|Android/i.test(navigator.userAgent||'')}
function voiceNameMatches(v){return /dari|persian|farsi|فارسی|دری/i.test((v&&v.name)||'')}
function pickVoice(lang){
  ...
  // Mobile phones usually expose Persian as fa-IR, not Dari fa-AF.
  // The previous fa-AF-only path can silently fail on mobile.
  if(isMobile()){
    return vs.find(v=>v.lang==='fa-IR')||
           vs.find(v=>v.lang==='fa-AF')||
           vs.find(v=>v.lang&&v.lang.toLowerCase()==='fa')||
           vs.find(v=>v.lang&&v.lang.toLowerCase().startsWith('fa'))||
           vs.find(voiceNameMatches);
  }
...
function preferredLang(lang,voice){
  if(voice&&voice.lang)return voice.lang;
  if(lang==='de')return 'de-DE';
  if(lang==='en')return 'en-US';
  return isMobile()?'fa-IR':'fa-AF';
}
...
function unlockSpeech(){updateVoiceStatus();els.now.textContent='Voice list refreshed.'}
function detectDevice(){let d='Desktop/laptop';const ua=navigator.userAgent||'';if(/iPhone|iPad|iPod/i.test(ua))d='iPhone/iPad';else if(/Android/i.test(ua))d='Android';else if(/Windows/i.test(ua))d='Windows PC';else if(/Macintosh/i.test(ua))d='Mac';els.deviceInfo.textContent=`Detected: ${d}. Dari uses mobile-aware Persian/Farsi browser voice selection.`}
