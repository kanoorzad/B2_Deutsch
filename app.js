...
const STORE='b2-native-cards-extra-v33';
...
function pickVoice(lang){const vs=voices();if(lang==='de')return vs.find(v=>v.lang==='de-DE'&&/premium|enhanced|natural|neural|online|siri|google|microsoft|premium compact|eloquence/i.test(v.name))||vs.find(v=>v.lang==='de-DE')||vs.find(v=>v.lang&&v.lang.startsWith('de'));if(lang==='en')return vs.find(v=>v.lang==='en-US'&&/premium|enhanced|natural|neural|online|siri|google|microsoft|premium compact|eloquence/i.test(v.name))||vs.find(v=>v.lang==='en-US')||vs.find(v=>v.lang&&v.lang.startsWith('en'));return vs.find(v=>v.lang==='fa-AF')||vs.find(v=>v.lang==='fa-IR')||vs.find(v=>v.lang&&v.lang.toLowerCase().startsWith('fa'))}
function voiceLabel(v){return v?`${v.name} (${v.lang})`:'Browser default voice'}
function getSpriteMeta(){return window.DARI_AUDIO_SPRITE||{sprites:['dari-sprite-1.mp3?v=33'],entries:{}}}
function spriteSrcFor(entry){const meta=getSpriteMeta();const list=meta.sprites||[];const n=Math.max(1,Number(entry&&entry.f)||1);return list[n-1]||list[0]||'dari-sprite-1.mp3?v=33'}
...
function sayWebSpeech(text,lang='de',done=()=>{}){if(!text){done();return}if(!('speechSynthesis'in window)){done();return}const u=new SpeechSynthesisUtterance(text);u.lang=lang==='de'?'de-DE':lang==='en'?'en-US':'fa-AF';u.rate=lang==='de'?0.78:0.86;u.pitch=1;const v=pickVoice(lang);if(v)u.voice=v;u.onend=done;u.onerror=done;speechSynthesis.speak(u)}
...
function detectDevice(){let d='Desktop/laptop';const ua=navigator.userAgent||'';if(/iPhone|iPad|iPod/i.test(ua))d='iPhone/iPad';else if(/Android/i.test(ua))d='Android';else if(/Windows/i.test(ua))d='Windows PC';else if(/Macintosh/i.test(ua))d='Mac';els.deviceInfo.textContent=`Detected: ${d}. Dari/Farsi uses high-quality local MP3 sprites; German/English use browser voices.`}
function updateVoiceStatus(){if(!('speechSynthesis'in window)){els.voiceDe.textContent=els.voiceEn.textContent='Speech not supported'}else{els.voiceDe.textContent=voiceLabel(pickVoice('de'));els.voiceEn.textContent=voiceLabel(pickVoice('en'))}els.voiceFa.textContent=dariSpriteUnlocked?'Dari audio sprites unlocked':'Dari high-quality local MP3 sprites'}
